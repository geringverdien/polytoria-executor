...
struct NetTracer
{
    ...
    static void InvokeServerHook(NetworkEvent *evt, NetMessage *message)
    {
        ...
        if (NetTracer::exec != nullptr)
        {
            DynValue *callback = NetTracer::exec->GetRegistry()->GetItem((UO *)US::New("InvokeServerCallback"));
            if (callback != nullptr)
            {
                Closure *closure = callback->GetFunction();
                if (closure != nullptr)
                {
            ...
            {
                std::cout << "Callback is empty wthhh " << std::endl;
            }
